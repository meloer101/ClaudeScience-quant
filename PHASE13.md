...
**代码改动**：
- [ ] 新增 `skills/sandbox.py`：把 `compute()` 执行搬进子进程。
  - 子进程 entrypoint 用 `resource.setrlimit` 设 `RLIMIT_CPU`（CPU 秒）、`RLIMIT_AS`（地址空间/内存）、`RLIMIT_FSIZE`（写文件大小）；父进程侧 `subprocess` 超时（wall-clock）兜底。
  - **网络隔离**：子进程环境剥离网络能力（最低限度不给任何 socket 库入口；`compute()` 本就只该吃 pandas/numpy，[codeexec.py](quantbench/skills/codeexec.py) 的 `_SAFE_BUILTINS` 已禁 `__import__`，子进程内继续沿用）。
  - **文件系统白名单**：子进程 cwd 设为临时目录，只读挂数据目录、只写 artifact 目录（通过传参而非隐式 cwd 访问）。
  - 数据传递：父子间用序列化（parquet/pickle over pipe 或临时文件）传 `data_df` 与返回的 `Series`；保持 [run_signal_code](quantbench/skills/codeexec.py:14) 的签名不变，内部改走子进程。
- [ ] 资源上限写入 config（`SandboxConfig(cpu_seconds, mem_mb, wall_timeout_s, max_write_mb)`），默认值保守，manifest 记录实际用值。
- [ ] 超时/超限 → 结构化错误（`{"error": "sandbox: CPU limit exceeded"}`），Coordinator 当作工具失败处理，不崩溃整个 run。
- [ ] （加分）资源用量（峰值内存、CPU 秒、wall time）回填 manifest，为 5.4 成本可观测性铺路。

**验收**：合成一个死循环因子（`while True: pass`）→ 命中 CPU rlimit、返回结构化错误、run 不挂；合成一个内存炸弹（`np.zeros((10**9,))`）→ 命中 `RLIMIT_AS`；一个正常因子行为与旧路径**数值等价**（同一 golden run 的 signal 输出不变）。子进程无法写数据目录、无法读 artifact 目录外的文件。
