...
**缺什么**：
- [x] ~~插件化 provider 接口（自研）~~ → **已决策并落地只读 MCP client 方案，见 7.2**：用户通过外部 MCP server 接入自己的数据源/工具，无需自研插件协议。
- [ ] 内置一等 provider（有质量校验、可获信任等级的数据源）仍走 `providers/base.py` 抽象，与 MCP 外部工具（untrusted）区分信任层级。

### 4.5 执行沙箱（优先级：🟡 中高，生产化前必须还的债）

**现状**：`run_signal_code` 与截面 `run_signal_code_panel`（`codeexec.py`）已把 `compute()` 的执行搬进 `resource.setrlimit` 约束的独立子进程（`skills/sandbox.py`），不再是同进程 `exec()`。CPU 时间与父进程 wall-clock 兜底在 Linux/macOS 均生效；`RLIMIT_AS` 内存上限在 Linux 上生效，在 macOS/Darwin 上因内核限制无法通过 `setrlimit` 设置，做了显式降级（不静默假装生效）。沙箱调用用量会进入 manifest 的 `sandbox_usage`。仍未做 Docker 级的文件系统/网络隔离。

**缺什么**：
- [x] 受限子进程 + rlimit 沙箱执行模型生成代码（`run_signal_code` 路径）；结构化超限错误，run 不崩溃。
- [x] 超时（wall-clock 兜底）/CPU 上限；内存上限（Linux 生效，macOS 已知限制并显式探测降级）。
- [x] 文件系统访问白名单降级处理：当前靠 builtins 黑名单禁 `open`/`__import__`，并把子进程 cwd 切到临时目录作为纵深防御；未做真正的只读/只写目录挂载。
- [x] 交叉截面/`screen_factors` 路径已接入沙箱：整个 panel 一次进子进程，子进程内按 symbol 分组执行 `compute`，引擎只消费预计算 `factor_values`。
- [ ] Docker（作为子进程隔离不足时的后续增量，本阶段未采用）。
...
- MCP client 接入外部数据工具（7.2，替代原 4.4 的自研插件接口方案）✅ 只读数据工具已完成
...
**任务**：
- [x] 配置文件声明外部 MCP server（本阶段支持本地 stdio transport、启用的 tool 白名单）。
- [x] `MCPClientManager`：基于官方 `mcp` Python SDK，把 MCP server 的 tool 动态注册进 `SkillRegistry`，Coordinator 无感知。
- [x] **审计红线一**：每次 MCP tool 调用（server 名、tool 名、参数、结果 hash）记入 manifest，处理方式类比 `injected_skills`。
- [x] **审计红线二**：MCP tool 返回的数据未经内置数据质量校验，不享受内置 provider 的信任等级。使用了外部 MCP 数据的 run 由 Reviewer 打上 `external_data_unverified` 信息标签，verdict 逻辑不受影响。
- [x] MCP tool 的白名单默认只读（数据获取类）；执行类 / 有副作用的 MCP tool 本阶段拒绝注册，后续等任意 RPC 沙箱设计后再放开。
