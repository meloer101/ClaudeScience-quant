...
| GAP 条目 | 状态 | 本计划交付 |
|---|---|---|
| 4.1 多轮 session | ❌ 未动 | §1.4 |
| 4.2 计划确认 | ❌ 未动 | §1.3 |
| 4.3 文献接入 | ❌ 未动 | §1.5 |
| 4.4 可扩展性（→MCP client, GAP 7.2） | ❌ 未动 | §1.1 |
| 4.4 附属：SKILL.md 对齐（GAP 7.3） | ❌ 未动 | §1.2 |
| 4.5 沙箱 | 🟡 核心完成，三个收尾项 | §1.0 |
| 4.6 远程计算 | ❌ 未动（仅接口预留） | §1.6 |
...
- [ ] 依赖：官方 `mcp` Python SDK（唯一新增依赖，PHASE13 已批准）。
- [ ] 配置：`PROJECT_ROOT/mcp_servers.json`（走 [config.py](quantbench/config.py) 的 PROJECT_ROOT 惯例）：`[{name, transport: {command|url}, enabled_tools: [...], allow_write: false}]`。`enabled_tools` 是**显式白名单**，缺省为空 = 一个都不注册（不是"全部"）。
- [ ] 新增 `skills/mcp_adapter.py`：启动时连接各 server、`list_tools`、把每个白名单内 tool 包成 [Skill](quantbench/skills/registry.py:7)（`name=f"mcp_{server}_{tool}"`，`inputSchema` 直接透传为 params schema），注册进 [SkillRegistry](quantbench/skills/registry.py:14)。`registry.execute` 命中时转发 `call_tool`。Coordinator 无感知——工具形状与内置完全一致，[loop.py](quantbench/agent/loop.py) 不改一行。
- [ ] **审计红线一**：每次调用记 `ctx.mcp_calls.append({server, tool, args, result_sha256})`，manifest 新段 `mcp_calls`（模式照抄 `delegations`，[store.py](quantbench/artifact/store.py) 加一个参数）。
- [ ] **审计红线二**：`ctx.mcp_calls` 非空的 run，`run_review` 注入 `external_data_unverified` finding（severity=`info`，**不封顶 verdict**——它是来源标签不是质量缺陷；实现照抄 [report.py](quantbench/review/report.py) 现有 optional-detail finding 的模式）。
- [ ] 拒绝逻辑：`allow_write: true` 或 tool 名/描述含副作用信号的,注册时直接拒绝并 warning（宁可误拒——用户可改配置显式放行,但本阶段放行路径直接抛 `NotImplementedError` 指向沙箱依赖）。
...
### 1.3 计划确认 / human-in-the-loop（GAP 4.2，🟡 中——先于 session，是它的踏脚石）

- [ ] Coordinator 两阶段：新增 `Coordinator.plan(user_request) -> ResearchPlan`（结构化：`{steps, data_needs, assumptions: {cost_bps, execution, neutralize, universe, date_range}, expensive_tools: [...]}`）。实现为一次**不带工具的受限 LLM 调用**（或 `max_turns=1` 的 SubAgent——复用 [subagent.py](quantbench/agent/subagent.py)，第三个实例），输出 JSON schema 校验。
- [ ] 执行阶段接受 `plan_overrides: dict`（用户改过的假设），注入 [execute()](quantbench/agent/coordinator.py:641) 的首条 user message：「计划已确认，假设如下…」。
- [ ] CLI（[cli.py](quantbench/cli.py)）：交互式展示计划 → `y/n/编辑参数`；`--yes` 跳过（CI/脚本零回归——**默认行为不变，确认模式是新旗标 `--plan` opt-in**，与 PHASE13"默认开启"的原设想相比这是刻意收窄：CLI 的存量用户全是脚本/测试，破坏性默认得不偿失；Web 端才是确认交互的主场）。
- [ ] API：`POST /api/runs` 增 `plan_first: bool`；新增 `POST /api/runs/{id}/plan/confirm`（body 携带 overrides）。run 状态机加一个 `awaiting_confirmation` 态。
- [ ] **审计**：manifest 新段 `plan`（原计划）+ `plan_overrides`（用户改动）+ `plan_execution_diff`（计划 vs 实际调用的工具/参数差异，从 `run.steps` 事后 diff 得出）。

**验收**：CLI `--plan` 跑截面研究,看到结构化计划、改 `cost_bps` 后确认,run 按改后值执行且 manifest 三段齐全;`--yes` 与不带旗标的行为和旧版逐字节一致;API 的 `awaiting_confirmation` 态可被 confirm 推进。

### 1.4 多轮对话式 session（GAP 4.1，🟡 本阶段最大产品跃迁,依赖 1.3）

- [ ] Session 模型（新 `quantbench/api/session.py` + artifact 持久化）：session = `{session_id, turns: [{turn_index, user_message, run_id|None, summary}]}`,存 `runs/_sessions/{session_id}.json`（复用 artifact 目录惯例,不引数据库）。
- [ ] 上下文延续：新一轮的 messages 组装时,在 system 之后注入「本 session 已有 run 的结构化摘要」（每 run：hypothesis、verdict、关键 metrics、run_id）——**不是**塞历史 messages（[execute() docstring](quantbench/agent/coordinator.py:641) 已记录过全量历史会污染 skill 匹配与 context 的顾虑）。
- [ ] 追问语义：「把手续费改成 10bps 再看看」→ Coordinator 在有 session 上下文时优先走 [run_fork](quantbench/agent/coordinator.py:604)（复用 [build_fork_config](quantbench/library/fork.py)）,parent 定位为摘要中最近的 run。给模型一个新工具 `fork_previous_run(run_id, modification)` 而不是靠 prompt 猜——工具化的路由可测、可审计。
- [ ] API：`POST /api/sessions`、`POST /api/sessions/{id}/turns`（返回 run_id,SSE 复用 [stream_run_events](quantbench/api/server.py:146)）、`GET /api/sessions/{id}`。
- [ ] Web（[App.tsx](web/src/App.tsx)）：v1 做「对话流 + run 卡片」的最小版——消息列表 + 每轮内嵌现有 run 详情组件;1.3 的计划卡片嵌入此流。样式增量,不重写现有页面。
- [ ] **溯源**：manifest 记 `session_id` + `turn_index`;实验库（library）查询支持按 session 聚合。
...
- [ ] `ExecutionBackend` 协议（`local` / `remote`）;`screen_factors` 的 fan-out 经该抽象派发。`local` = 现状线程池,`remote` 抛 `NotImplementedError("remote execution is planned; use backend=local")`。
- [ ] config 留 `execution_backend: "local"` 键与 schema 位。

**验收**：默认行为逐字节不变;配置切 remote 得到清晰报错而非静默回退。
...
| 计划两阶段 (1.3) | `plan` / `plan_overrides` / `plan_execution_diff` | — | 无 |
...
- 不做「计划确认默认开启」的破坏性默认（CLI opt-in,Web 主推）。
...
*落地后回填本文件与 [GAP_ANALYSIS.md](GAP_ANALYSIS.md) 第四章勾选项,PHASE13.md 的 1.2–1.7 标注「细化并落地于 13B」。*
